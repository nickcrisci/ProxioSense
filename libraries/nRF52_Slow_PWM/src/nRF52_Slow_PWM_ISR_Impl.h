...
#ifndef NRF52_SLOW_PWM_ISR_IMPL_H
#define NRF52_SLOW_PWM_ISR_IMPL_H
...
NRF52_Slow_PWM_ISR::NRF52_Slow_PWM_ISR()
  : numChannels (-1)
{
}
...
void NRF52_Slow_PWM_ISR::init()
{
  ...
  for (uint8_t channelNum = 0; channelNum < MAX_NUMBER_CHANNELS; channelNum++)
  {
    ...
    PWM[channelNum].pin      = INVALID_NRF52_PIN;
  }
...
int NRF52_Slow_PWM_ISR::findFirstFreeSlot()
{
  // all slots are used
...
int NRF52_Slow_PWM_ISR::setupPWMChannel(const uint32_t& pin, const uint32_t& period, const float& dutycycle,
...
bool NRF52_Slow_PWM_ISR::modifyPWMChannel_Period(const uint8_t& channelNum, const uint32_t& pin, const uint32_t& period,
...
void NRF52_Slow_PWM_ISR::deleteChannel(const uint8_t& channelNum)
{
  ...
  if ( (PWM[channelNum].pin != INVALID_NRF52_PIN) && (PWM[channelNum].period != 0) )
  {
    memset((void*) &PWM[channelNum], 0, sizeof (PWM_t));
...
void NRF52_Slow_PWM_ISR::restartChannel(const uint8_t& channelNum)
{
  if (channelNum >= MAX_NUMBER_CHANNELS)
  {
    return;
...
bool NRF52_Slow_PWM_ISR::isEnabled(const uint8_t& channelNum)
{
  if (channelNum >= MAX_NUMBER_CHANNELS)
  {
    return false;
...
void NRF52_Slow_PWM_ISR::enable(const uint8_t& channelNum)
{
  if (channelNum >= MAX_NUMBER_CHANNELS)
  {
    return;
...
void NRF52_Slow_PWM_ISR::disable(const uint8_t& channelNum)
{
  if (channelNum >= MAX_NUMBER_CHANNELS)
  {
    return;
...
void NRF52_Slow_PWM_ISR::enableAll()
{
...
void NRF52_Slow_PWM_ISR::disableAll()
{
...
void NRF52_Slow_PWM_ISR::toggle(const uint8_t& channelNum)
{
  if (channelNum >= MAX_NUMBER_CHANNELS)
  {
    return;
...
int8_t NRF52_Slow_PWM_ISR::getnumChannels()
{
  return numChannels;
}

#endif    // NRF52_SLOW_PWM_ISR_IMPL_H
